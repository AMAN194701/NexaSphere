  ...
  return (
    <div style={{ padding: '2rem', background: '#1e293b', borderRadius: '16px', color: 'var(--t1)' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
          <label style={{ display: 'block', marginBottom: '0.5rem' }}>Select Event to Mark Attendance For</label>
          <input 
      ...
      {eventId && (
          <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
              <div style={{ flex: '1', minWidth: '300px' }}>
                  ...
                  <div id="reader" style={{ width: '100%', maxWidth: '400px', background: '#fff', borderRadius: '8px', overflow: 'hidden' }}></div>
              </div>
              
              <div style={{ flex: '1', minWidth: '300px' }}>
                  <h3>Fallback Manual Entry</h3>
                  <form onSubmit={handleManualEntry} style={{ background: 'rgba(255,255,255,0.05)', padding: '1.5rem', borderRadius: '8px' }}>
                      <input 
                          type="email" 
                          name="email" 
                          className="input-field" 
                  ...
                  {scanResult && (
                      <div style={{ 
                          ...
                          {scanResult.type === 'success' ? (
                              <div>
                                  <p><strong>Name:</strong> {scanResult.data.full_name}</p>
                                  <p><strong>Email:</strong> {scanResult.data.email}</p>
                                  {scanResult.data.already_attended && <p style={{ color: '#f59e0b' }}>Note: Participant had already marked attendance.</p>}
                              </div>
                          ) : (
