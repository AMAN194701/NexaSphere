...
import crypto from "crypto";
import {
  coreTeamMemberSchema,
  normalizeCoreTeamGate,
} from "../schemas/coreTeamMemberSchema.js";
import { UnauthorizedError } from "../utils/errors.js";
...
export const coreTeamService = {
  async listMembers() {
    ...
    if (cached !== undefined) {
      console.log(`[Core Team Service] Cache HIT for key "${cacheKey}"`);
      return cached;
    }

    console.log(
      `[Core Team Service] Cache MISS for key "${cacheKey}". Fetching from source.`
    );
  ...
  async updateMember(id, input) {
    ...
    return res;
      ...
      content.coreTeam.push(newMember);
      await writeContent(content);
      added = sanitizeCoreTeamMemberRecord(newMember);
    }

    // Invalidate distributed cache after database mutation
    await cacheService.invalidateCache("core_team");
    return added;
  },
  ...
  async assertCanManageActivityEvent(body) {
    ...
    if (gate.password !== expectedPassword) {
      throw new UnauthorizedError(
        "Unauthorized. Core team details or password did not match."
      );
    }
